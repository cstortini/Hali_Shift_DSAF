...
polcrib1 <- st_as_sf(hercrib, coords = c("longitude", "latitude"), crs = "WGS84")
# Reproject both datasets to align their CRS (e.g., EPSG:4326)
polcrib1 <- st_transform(polcrib1, crs = st_crs(NAFO))
# Perform a spatial join to add the NAFO zone info to each point in halcrib
polcrib_with_zones <- st_join(polcrib1, NAFO)

# Check the first few rows of the resulting dataset
head(polcrib_with_zones)
names(polcrib_with_zones)
unique(polcrib_with_zones$ZONE)
# Remove rows where NAFO_ID or ZONE are NA
halcrib_with_zones_clean <- polcrib_with_zones
...
halcrib_with_zones_clean1 <- halcrib_with_zones_clean %>%
  ...
  filter(!is.na(NAFO_Zones))

south_to_north <- c("5ZY","4X","4VW","3NOPs","4RST","3KL","2JHG","0AB")
...
# ── Save ──────────────────────────────────────────────────────────────────────
ggsave(
  filename = "CRIB results/COSEWIC/Herring_CRIB_Full_and_3Indices_ByNAFO.png",
  plot     = final_plot,
  width    = 18,
  height   = 10,
  dpi      = 400,
