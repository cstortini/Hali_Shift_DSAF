...
Assembled in **Prepare data for POC.R**, the **Data_for_POC folder** contains data for the temporal (and spatial where applicable*) trends for the 6 shift indicators, calculated based on Atlantic halibut abundance estimates form SDM output. All data have been subset to spring estimates.

1. Abundance*: trend in total estimated abundance  
...
POC_Abundance<-read.csv(here::here("Data/Data_for_POC/POC_Abundance.csv"))
str(POC_Abundance)

```

### Trends in Abundance Estimate 
...
POC_AO<-read.csv(here::here("Data/Data_for_POC/POC_AreaOccupied.csv"))
str(POC_AO)
POC_AO_sub<-subset(POC_AO, POC_AO$Threshold ==90)
```

### Trends in : Total area occupied ( 90% of abundance), Percent of the total area used, Area Efficiency  
...
POC_AWD<-read.csv(here::here("Data/Data_for_POC/POC_AWD.csv"))
str(POC_AWD)

```

### Trends in Abundance-Weighted Depth
...
RE_DAT<- read.csv(here::here("Data/Data_for_POC/POC_RangeEdge.csv"))
str(RE_DAT)
```

### Trends in Leading and Trailing Edges 
...
* centroid_latitude_Median & centroid_longitude_Median: weighted median

* centroid_latitude_Quantile_05 & centroid_longitude_Quantile_05: weighted 5th percentile 

* centroid_latitude_Quantile_95 & centroid_longitude_Quantile_95: weighted 95th percentile

```{r COG data, echo=FALSE}
POC_COG<-read.csv(here::here("Data/Data_for_POC/POC_COG.csv"))
str(POC_COG)

```

### Trends in Centre of Gravity (Mean)
...
POC_DtoB<-read.csv(here::here("Data/Data_for_POC/POC_DtoB.csv"))
str(POC_DtoB)

```

### Trends in Distance to Shared Border 
